...
const obtener_recinto_id = async () => {
    let resuldatos;
    //Obtenemos los datos
    const id_recinto = window.localStorage.getItem('recinto_editar');

    await obtener_recinto(id_recinto).then(res => { resuldatos = res });

    //resuldatos.geolocalizacion
    input_nombre_recinto.value = resuldatos.datos.nombre_recinto;
    img_imagen.src = resuldatos.datos.fotos[0];

    txt_area_direccion_exacta.value = resuldatos.datos.direccion_exacta;
    input_cap_tradicionales.value = resuldatos.datos.capacidad_asientos_tradicionales;
    input_cap_especiales.value = resuldatos.datos.capacidad_asientos_especiales;
    input_capacidad.innerHTML = `Capacidad total: ${resuldatos.datos.capacidad}`;
    sl_encargado_recinto.value = resuldatos.datos.encargado_asociado_id._id;


    sl_provincia.value = establecer_localizacion(sl_provincia.options,resuldatos.datos.provincia);
    await obtener_cantones(sl_provincia.value);
    sl_canton.value = establecer_localizacion(sl_canton.options,resuldatos.datos.canton);
    await obtener_distritos(sl_canton.value);
    sl_distrito.value = establecer_localizacion(sl_distrito.options,resuldatos.datos.distrito);
};

const establecer_localizacion = (coleccion, texto) => {
    for (let i=0; i<coleccion.length;i++) {
        if (coleccion[i].childNodes[0].nodeValue === texto){
            return i;
        }
    }
    return undefined;
