...
const contenedor = document.querySelector('#proximos');
...
let proximos = async (anno_obt) => {
    let annotest = [];
    let pro;
    for (let i = 0; i < anno_obt.length; i++) {

        pro = anno_obt[i].slice(6, 10);
        annotest.push(pro);
...
let crearCards = async () => {
    ...
    for (let i = 0; i < lista_eventos.length; i++) {
        let anno = await proximos(lista_eventos[i]['fechas']);
        if (anno == '2019') {
